...
const createDelivery = (deliverySystem) => {
    ...
    for (let i = 0; i < deliverySystem.length; i++) {
        ...
        <div class="delivery">
        <div>Delivery man name: ${deliverySystem[i].name}</div>
        ...
        <button id="remove-btn" class="btn btn-danger" onclick="remove('${deliverySystem[i].email}')" style="margin-left: 50px; cursor: pointer;">Remove Delivery man</button>
    </div>
        `
    }
}
...
const remove = (email) => {
    fetch('/deleteDelivery', {
        ...
        .then(response => {
            if(response == 'success'){
                showAlert('Delivery man deleted successfully!', 'success')
            }
        })
}
