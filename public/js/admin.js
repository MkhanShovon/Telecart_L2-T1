...
const deliveries = document.querySelector('#delivery-num')
const onlyCustomers= document.querySelector('#only-customer-num')
...
fetch('/delivery-num', {
        ...
        .then(data => {
            console.log(data);
            deliveries.innerHTML = data;
        })

fetch('/only-customer-num', {
    method: 'post',
    headers: new Headers({ 'Content-type': 'application/json' }),
}).then(res => res.json())
    .then(data => {
        console.log(data);
        onlyCustomers.innerHTML = data;
    })